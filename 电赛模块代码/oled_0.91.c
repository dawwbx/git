 ...
 * @note
 * ???????:
 * 1. STM32?????IIC???????OLED_Init()?????OLED. ???STM32??????OLED????, ????20ms??????OLED
 * 2. ????OLED_NewFrame()????????????
 * 3. ????OLED_DrawXXX()??к?????????ε???? ????OLED_Printxxx()??к???????????????
 * 4. ????OLED_ShowFrame()??????????????OLED
 *
 * @note
 * ??????????????? ????????????????????UTF-8
 *
...
/**
 * @brief ????????? ????????????
 * @param ColorMode ?????COLOR_NORMAL/COLOR_REVERSED
 * @note ??????????????????????
 */
...
void OLED_ShowFrame()
{
  ...
  {
    OLED_SendCmd(0xB0 + i); // ????????
    OLED_SendCmd(0x00);     // ?????е????4λ
    OLED_SendCmd(0x10);     // ?????е????4λ
 ...
 * @param end ????λ
 * @param color ???
 * @note ???????????е????????startλ????endλ???????data???
 * @note start??end???Χ?0-7, start????С?????end
 * @note ???????OLED_SetByte_Fine?????????????????????????е?????????
 */
 ...
 * @param len λ??
 * @param color ???
 * @note ???????????д?(x,y)?????????lenλ???????data???
 * @note len???Χ?1-8
 * @note ???????OLED_SetByte_Fine???????????????????????????????????????λ??, ??????????????????????(???)
 */
 ...
 * @param h ???
 * @param color ???
 * @note ???????????д?(x,y)?????w*h???????????data?е?????
 * @note data????????ò????????????
 */
void OLED_SetBlock(uint8_t x, uint8_t y, const uint8_t *data, uint8_t w, uint8_t h, OLED_ColorMode color)
{
  uint8_t fullRow = h / 8; // ??????????
  uint8_t partBit = h % 8; // ????????????е???Чλ??
 ...
 * @note ??????????е???????????????, ??:
 * 1. ????????????????UTF-8
 * 2. ??ò??????LED?????????????(https://led.baud-dance.com)
 */
/**
 * @brief ?????????
 * @param x ??????????
...
//初始化屏幕OLED_Init();
//刷新屏幕OLED_NewFrame();
//使用前需要先在oled_font.c中定义字体
//OLED_PrintString(13, 0, "样例文字", &font16x16, OLED_COLOR_NORMAL);在屏幕上13为x坐标，0为y坐标，"样例文字"为要显示的文字，&font16x16为字体，OLED_COLOR_NORMAL为颜色
// OLED_DrawRectangle(13,40,101,12,OLED_COLOR_NORMAL); //13为x坐标，40为y坐标，101为宽度，12为高度上绘制矩形
// OLED_DrawFilledRectangle(13,40,j,k,OLED_COLOR_NORMAL); //j为屏幕宽度，k为高度，13为x坐标，40为y坐标上填充矩形
//  OLED_ShowFrame(); 设置完所有字体后调用此显示在屏幕
