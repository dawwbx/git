...
void Emm_V5_Vel_Control(uint8_t addr,int16_t vel, uint8_t acc, bool snF); // 速度模式控制
void Emm_V5_Pos_Control(uint8_t addr, uint16_t vel, uint8_t acc, int32_t clk, bool raF, bool snF); // 位置模式控制
...
void Emm_V5_Getposition(uint8_t addr);// 获取位置
void Emm_V5_GetSpeed(uint8_t addr);// 获取位置
